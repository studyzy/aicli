...
**注意**：默认情况下，aicli 会在执行前显示翻译后的命令（输出到 stderr），这有助于您学习实际的 shell 命令。命令提示不会干扰管道操作或输出重定向。
...
aicli 遵循 Unix 的输出流约定：

- **stdout（标准输出）**：命令执行的结果（用于管道传递给其他命令）
- **stderr（标准错误）**：状态信息、翻译后的命令、错误提示

```bash
# 示例：命令提示输出到 stderr，结果输出到 stdout
$ aicli "显示1到5的数字"
💡 执行命令: seq 1 5    # 这一行在 stderr
1                       # 这些行在 stdout
2
3
4
...
# 管道操作正常工作，因为只有 stdout 被传递
$ aicli "显示1到5的数字" | aicli "计算总和"
💡 执行命令: seq 1 5              # stderr：用户可见
💡 执行命令: awk '{s+=$1} END {print s}'  # stderr：用户可见
15                                 # stdout：在命令之间传递

# 需要纯净输出时，使用静默模式
$ aicli -q "显示1到5的数字" | wc -l
5                                  # 干净的输出，没有提示

# 只重定向结果，保持命令提示可见
$ aicli "列出文件" > output.txt
💡 执行命令: ls                   # 仍然在屏幕上可见
                                   # 文件列表保存到 output.txt
```

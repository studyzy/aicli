...
// NewSafetyChecker 创建新的安全检查器
func NewSafetyChecker(enableChecks bool) *SafetyChecker {
...
func (c *SafetyChecker) AddCustomPattern(pattern Pattern) {
	c.customPatterns = append(c.customPatterns, pattern)
}

// IsDangerous 检查命令是否危险
// 返回: 是否危险, 匹配的模式描述, 风险等级
func (c *SafetyChecker) IsDangerous(command string) (bool, string, RiskLevel) {
	if !c.enableChecks {
...
func (c *SafetyChecker) CheckMultiple(commands []string) (bool, []string, RiskLevel) {
	if !c.enableChecks {
...
// Enable 启用安全检查
func (c *SafetyChecker) Enable() {
	c.enableChecks = true
}

// Disable 禁用安全检查
func (c *SafetyChecker) Disable() {
	c.enableChecks = false
}

// IsEnabled 返回是否启用安全检查
func (c *SafetyChecker) IsEnabled() bool {
